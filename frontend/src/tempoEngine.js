/* eslint-disable */
// Auto-synced from front.html
export function initTempoEngine(container) {
        ...
        const itemsOn = (ds, opts = {}) => {
            const mmdd = ds.slice(5);
            const dObj = fromYmd(ds);
            const dayOfWeek = dObj.getDay(); // 0 = Dom, 1 = Lun, ..., 6 = Sáb
            const filterWork = opts.filterWork !== undefined ? opts.filterWork : (UI.showWork === false);

            return S.items.filter(it => {
                if (filterWork && (it.isWork || it.is_work)) return false;
        ...
        function getSortedItemsForDay(ds) {
            const items = itemsOn(ds);
            const curDay = ds || UI.selDay || todayY();
            const orderMap = (S && S.dayOrder && S.dayOrder[curDay]) || null;
            return [...items].sort((a, b) => {
                ...
                const ordA = a.orderIndex ?? a.order_index;
                const ordB = b.orderIndex ?? b.order_index;
        ...
        function moveItemInDay(id, direction, ds) {
            ds = ds || UI.selDay || todayY();
            const list = getSortedItemsForDay(ds);
            ...
            S.dayOrder = S.dayOrder || {};
            S.dayOrder[ds] = list.map(i => i.id);

            list.forEach((item, i) => {
                item.orderIndex = i;
                api.patchItem(item.id, { orderIndex: i }).catch(() => {});
            });
        ...
        const UI = {
            ...
            stack: null,
            calMode: 'month',
            showWork: localStorage.getItem('tempo.showWork') !== 'false'
        };
        ...
        function rowHTML(it, o = {}) {
            ...
            const isWork = !!(it.isWork || it.is_work);
            const workBadge = isWork ? `<span class="work-badge" title="Elemento de trabajo">💼 Trabajo</span>` : '';
            ...
            const reorderBtns = o.reorder ? `
            <div class="reorder-btns" role="group" aria-label="Cambiar posición">
              <button type="button" class="reorder-btn reorder-up ${o.isFirst ? 'is-disabled' : ''}" data-act="move-item-up" data-id="${it.id}" data-ds="${curDay}" title="Subir posición en este día" ${o.isFirst ? 'disabled' : ''}>
                <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.6"><path d="M18 15l-6-6-6 6"/></svg>
              </button>
              <button type="button" class="reorder-btn reorder-down ${o.isLast ? 'is-disabled' : ''}" data-act="move-item-down" data-id="${it.id}" data-ds="${curDay}" title="Bajar posición en este día" ${o.isLast ? 'disabled' : ''}>
                <svg viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="currentColor" stroke-width="2.6"><path d="M6 9l6 6 6-6"/></svg>
              </button>
            </div>` : '';
    ...
    <div class="fg">${lead}<button type="button" class="tx" data-act="edit" data-id="${it.id}" aria-label="Editar: ${esc(it.title)}"><span class="t1">${esc(titleDisplay)}${workBadge}</span>${o.cap ? `<span class="t2">${o.cap}</span>` : ''}</button>${flag}${meta ? `<span class="meta${o.hot ? ' hot' : ''}">${meta}</span>` : ''}${reorderBtns}${star}${chev}</div>
  </div>`;
        }
        ...
        function itemRow(it, o = {}) {
            if (it.kind === 'event') {
                ...
                return rowHTML(it, { ...o, meta, cap: capText, ds: o.ds });
            }
            if (it.kind === 'routine') {
                const repLabel = getRoutineLabel(it.repeat);
                const meta = (it.time ? fmtTime(it.time) + ' · ' : '') + repLabel;
                return rowHTML(it, { ...o, meta, cap: o.when || (it.notes ? esc(it.notes) : ''), ds: o.ds });
            }
            if (it.kind === 'reminder') {
                ...
                return rowHTML(it, { ...o, meta, cap: cp, hot, ds: o.ds });
            }
            if (it.kind === 'birthday' || /cumpleaños|cumple/i.test(it.title)) {
                const curDay = o.ds || UI.selDay || todayY();
                const curYear = parseInt(curDay.slice(0, 4), 10) || new Date().getFullYear();
                const bYear = it.year || (it.date && it.date.length === 10 ? parseInt(it.date.slice(0, 4), 10) : null);
                const age = bYear ? (curYear - bYear) : null;
                const nb = nextBday(it);
                const isExactDay = o.ds ? (it.date && (it.date.endsWith(o.ds.slice(5)) || it.date === o.ds)) : (nb.days === 0);
                const meta = isExactDay ? '¡hoy!' : (nb.days === 1 ? 'mañana' : (nb.days < 365 ? 'en ' + nb.days + ' d' : ''));
                
                const turns = (age != null && age > 0) ? age : nb.turns;
                let cp = '';
                if (turns != null && turns > 0) {
                    cp = `Cumple ${turns} años` + (nb.next ? ` · ${cap(fShort.format(nb.next))}` : '');
                } else if (nb.next) {
                    cp = cap(fShort.format(nb.next));
                }
                if (it.notes) {
                    cp = cp ? `${cp} · ${esc(it.notes)}` : esc(it.notes);
                }
                return rowHTML(it, { ...o, meta, cap: cp, hot: isExactDay || nb.days <= 1, ds: o.ds });
            }
            if (it.kind === 'wish') {
                const meta = it.price ? new Intl.NumberFormat('es-ES', { style: 'currency', currency: 'EUR', maximumFractionDigits: 2 }).format(it.price) : '';
                return rowHTML(it, { ...o, meta, cap: PRIO[it.prio || 0], star: true, chev: false, ds: o.ds });
            }
            let meta = o.timeline ? '' : (it.time ? fmtTime(it.time) : '');
            let cp = o.when || '';
            if (it.kind === 'shopping') {
        ...
        function renderCalCellHtml(d, ds, monthName, y) {
            ...
            const items = getSortedItemsForDay(ds);
            const dt = fromYmd(ds);
            ...
            items.forEach(i => {
                ...
                const isWork = !!(i.isWork || it_is_work(i));
                ...
                if (isBday) {
                    ...
                    const bYear = i.year || (i.date && i.date.length === 10 ? parseInt(i.date.slice(0, 4), 10) : null);
                    const targetY = parseInt(ds.slice(0, 4), 10) || new Date().getFullYear();
                    const age = bYear ? targetY - bYear : null;
                    titleText = age != null && age > 0 ? `🎂 ${name} (${age} años)` : `🎂 ${name}`;
                } else if (isWork) {
                    titleText = `💼 ${titleText}`;
                }
      ...
      ${chipsHtml}
        ...
        function it_is_work(i) {
            return Boolean(i && (i.isWork || i.is_work));
        }

        function renderCalHeaderBar(mainTitle, mode) {
            const showWork = UI.showWork !== false;
          ...
          <div class="cal-header-right">
            ...
            <label class="cal-work-toggle" title="${showWork ? 'Desactivar para ocultar eventos de trabajo' : 'Activar para ver eventos de trabajo'}">
              <input type="checkbox" id="chkCalWork" data-act="toggle-cal-work" ${showWork ? 'checked' : ''}>
              <span class="cal-work-pill"><span class="work-icon">💼</span><span class="work-text">Trabajo</span></span>
            </label>
          </div>
        </div>`;
        }
        ...
        function renderToday() {
            ...
            // --- Día seleccionado ---
            const ds = UI.selDay, list = getSortedItemsForDay(ds);
            ...
            const agenda = list.length ? `${holBanner}<div class="card">${list.map((i, idx) => itemRow(i, { ds, reorder: list.length > 1, isFirst: idx === 0, isLast: idx === list.length - 1 })).join('')}</div>`
  ...
  <button type="button" class="tp ${k === cur ? 'on' : ''}" data-tp="${k}" aria-pressed="${k === cur ? 'true' : 'false'}" style="--c:${kcol(k)};--cs:${ksoft(k)}">
    <span class="tpi" aria-hidden="true">${ic(v.icon, 18)}</span><span>${v.label}</span></button>`).join('')}</div>`;
        ...
        function fieldsHTML(kind, it, ctx) {
            const dateVal = kind === 'birthday'
                ? (it ? (it.date ? (it.date.length === 5 ? `${it.year || new Date().getFullYear()}-${it.date}` : it.date) : (ctx.date || todayY())) : '')
                : (it ? (it.date ? (it.date.length === 5 ? `${new Date().getFullYear()}-${it.date}` : it.date) : (ctx.date || todayY())) : (ctx.date || todayY()));
            let f = '';
            if (kind === 'event') {
    ...
    <div class="fgroup">
      ...
      <div class="frow"><label for="fd-loc">Ubicación</label><input type="text" id="fd-loc" name="location" placeholder="Lugar o dirección (opcional)" value="${esc(it?.location || '')}" autocomplete="off"></div>
                ...
                const selectedDays = (!isDaily && rep)
                    ? (rep.includes(',') ? rep.split(',').map(s => s.trim()) : [rep])
                    : ['mon'];
                f += `
    <div class="fgroup">
    ...
    <div class="routine-freq-wrap" id="routineFreqWrap">
      ...
      <div class="routine-days-container" id="routineDaysContainer" style="display:${isDaily ? 'none' : 'block'}">
        <div class="routine-days-label">Elegí los días en que se repite:</div>
        <div class="routine-days-grid" id="routineDaysGrid">
          ${[
            ...
            <button type="button" class="r-day-chip ${selectedDays.includes(day.d) ? 'on' : ''}" data-day="${day.d}" aria-pressed="${selectedDays.includes(day.d) ? 'true' : 'false'}" title="${day.full}">
    ...
    ${segHTML('prio', ['Algún día', 'Me interesa', 'Lo quiero'].map((l, v) => ({ v, label: l })), it?.prio ?? 0)}`;
      ...
      <div class="fgroup" style="margin-top:12px">
        <div class="frow"><label for="fd-qty">Cantidad</label><input type="text" id="fd-qty" name="qty" inputmode="text" placeholder="1 kg, ×2…" value="${it?.qty || ''}"></div>
            ...
            const isWork = it ? !!(it.isWork || it.is_work) : false;
            f += `
    <div class="fgroup" style="margin-top:10px">
      <div class="frow">
        <label for="fd-iswork" style="cursor:pointer;display:flex;align-items:center;gap:6px">
          <span style="font-size:16px">💼</span> <span style="font-weight:600">Clasificar como Trabajo</span>
        </label>
        <label class="switch-wrap" for="fd-iswork">
          <input type="checkbox" id="fd-iswork" name="isWork" ${isWork ? 'checked' : ''} aria-label="Clasificar como trabajo">
          <span class="switch-slider" aria-hidden="true"></span>
        </label>
      </div>
    </div>`;
        ...
        function openComposer(kind, editing, ctx = {}) {
            ...
            composer = { kind, editing: editing || null, pick: !ctx.fixedKind, ctx };
        ...
        document.addEventListener('change', e => {
            if (e.target && (e.target.id === 'chkCalWork' || e.target.dataset.act === 'toggle-cal-work')) {
                UI.showWork = e.target.checked;
                try { localStorage.setItem('tempo.showWork', String(UI.showWork)); } catch (_) { }
                refreshAll();
            }
        });
        sheetBody.addEventListener('click', e => {
            ...
            if (tp) {
                const keepTitle = sheetBody.querySelector('[name=title]')?.value ?? (composer.editing?.title || '');
                const keepDate = sheetBody.querySelector('[name=date]')?.value;
                const keepTime = sheetBody.querySelector('[name=time]')?.value;
                const keepEnd = sheetBody.querySelector('[name=end]')?.value;
                const keepLocation = sheetBody.querySelector('[name=location]')?.value;
                const keepNotes = sheetBody.querySelector('[name=notes]')?.value;
                const keepAlarm = sheetBody.querySelector('[name=alarm]')?.value;
                const keepPrice = sheetBody.querySelector('[name=price]')?.value;
                const keepQty = sheetBody.querySelector('[name=qty]')?.value;
                const keepIsWork = sheetBody.querySelector('#fd-iswork')?.checked;

                if (composer.editing) {
                    composer.editing.title = keepTitle;
                    if (keepDate) composer.editing.date = keepDate;
                    if (keepTime !== undefined && keepTime !== '') composer.editing.time = keepTime;
                    if (keepEnd !== undefined && keepEnd !== '') composer.editing.end = keepEnd;
                    if (keepLocation !== undefined) composer.editing.location = keepLocation;
                    if (keepNotes !== undefined) composer.editing.notes = keepNotes;
                    if (keepAlarm !== undefined && keepAlarm !== '') composer.editing.alarm = Number(keepAlarm);
                    if (keepPrice !== undefined && keepPrice !== '') composer.editing.price = parseFloat(keepPrice);
                    if (keepQty !== undefined) composer.editing.qty = keepQty;
                    if (keepIsWork !== undefined) composer.editing.isWork = keepIsWork;
                    composer.editing.kind = tp.dataset.tp;
                }
                composer.kind = tp.dataset.tp;
                buildSheet();
                const inp = sheetBody.querySelector('[name=title]');
                if (inp && keepTitle) inp.value = keepTitle;
            ...
            if (rDay) {
                const isCurrentlyOn = rDay.classList.contains('on');
                const allOn = sheetBody.querySelectorAll('.r-day-chip.on');
                if (isCurrentlyOn && allOn.length <= 1) {
                    return;
                }
                const nextState = !isCurrentlyOn;
                rDay.classList.toggle('on', nextState);
                rDay.setAttribute('aria-pressed', nextState ? 'true' : 'false');
                return;
            }
        ...
        function saveComposer() {
            ...
            it.isWork = !!sheetBody.querySelector('#fd-iswork')?.checked;
            if (kind === 'birthday') { it.date = fd.date.slice(5); it.year = Number(fd.date.slice(0, 4)) || null; }
                ...
                if (kind === 'event') {
                    ...
                    if (rMode === 'daily') {
                        ...
                        const dayChips = [...sheetBody.querySelectorAll('.r-day-chip.on')].map(b => b.dataset.day);
                        it.repeat = dayChips.length ? dayChips.join(',') : 'mon';
                    }
                    it.notes = fd.notes || '';
                }
      ...
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${P.check}</svg></button>`).join('')}</div>`;
        ...
        document.addEventListener('click', e => {
            ...
            if (act) {
                ...
                if (a === 'move-item-up') { moveItemInDay(id, -1, act.dataset.ds); return; }
                if (a === 'move-item-down') { moveItemInDay(id, 1, act.dataset.ds); return; }
...
export const mountTempoEngine = initTempoEngine;
